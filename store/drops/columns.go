...
type colSet struct {
	...
	// i64 holds the bigint columns. They exist for one reason: PostgreSQL has
	// no unsigned integer type, and a Go uint32 does not fit in `integer` —
	// see add's reflect.Uint32 case, and [auth.Credential.SignCount], the
	// field that needs it.
	...
	default:
		panic(fmt.Sprintf(
			"authlayer/store/drops: column %q has unsupported Go type %s; supported: "+
				...
				"int, int32, uint32, and named types whose underlying type is string, "+
				"int, int32 or uint32", name, ft))
	}
