...
Not exported by `scope`. `ErrEmptyPermissions` **is** `apikey.ErrEmptyPermissions`,
aliased. Denials are not re-declared: a management or consent call the actor
may not make is `scope.ErrForbidden` or `scope.ErrNotMember`, and minting above
one's standing is `scope.ErrPrivilegeEscalation` — none of which reaches a token
endpoint.

For these the table carries a fourth column: the RFC 6749 §5.2 (or RFC 8628 §3.5,
RFC 6750 §3.1, RFC 7591 §3.2.2) `error` code `oauth.ErrorCode(err)` returns
beside the status, for the JSON body your token endpoint writes. The walk is in
precedence order, so a wrapped chain answers for its most specific member.

| Error | Meaning | `error` code | Suggested status |
|---|---|---|---|
| `ErrInvalidToken` | `Authenticate` refused the access token — wraps the cause: a signer sentinel, `ErrGrantRevoked`, `ErrGrantExpired`, `ErrGrantNotFound`, `ErrClientDisabled`, `ErrClientNotFound`, or a token that is not this package's | `invalid_token` (RFC 6750) | 401 |
...
| `ErrInvalidClientMetadata` | A `ClientSpec` or registration this package cannot honour | `invalid_client_metadata` (RFC 7591) | 400 |
...
| `ErrDeviceNotPending` | Already approved, denied or redeemed — or the compare-and-set lost | `invalid_request` | 409 |
...
<Note>
The token endpoints never show a client a not-found: they wrap it in
`ErrInvalidGrant`, because RFC 6749 does not tell a client whether its code was
unknown or spent. `Introspect` returns **no error at all** for an invalid token
— see [Introspection & revocation](/agents/introspection-and-revocation).
</Note>
...
|---|---|---|
| duplicate `token_hash` (session, verification, invite, API key, authorization code, device code, refresh token) | `memory.ErrTokenHashTaken` | the driver's unique violation |
| duplicate device-flow `user_code` | `memory.ErrUserCodeTaken` | the driver's unique violation |
